...
A from-scratch autograd engine and neural network library, built by following
[Andrej Karpathy's micrograd](https://github.com/karpathy/micrograd) tutorial.

The goal of this project wasn't to produce a novel library — it's a learning
exercise focused on understanding exactly how backpropagation and reverse-mode
automatic differentiation work under the hood, without relying on PyTorch or
...
into multiple downstream operations — overwriting would silently break the
...
computation (same inputs, weights, and bias) in `demo.ipynb` — the two match
to floating-point precision. This was the most useful sanity check while
